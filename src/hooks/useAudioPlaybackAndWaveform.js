 ...
 */
const useAudioPlaybackAndWaveform = (
  audioRef,
  canvasRef,
  audioDuration,
  waveformData,
  ...
  const getDuration = () => {
    return (
      getValidDuration(audioDuration) ||
      getValidDuration(audioRef.current.duration)
    );
  };
  ...
  const handleWaveformClick = useCallback(
    ...
    [audioRef, waveformData, drawStaticBars, getPositionFromEvent, getDuration]
  );
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      setTimer(
        `${String(minutes).padStart(1, '0')}:${String(seconds).padStart(
          2,
          '0'
        )}`
      );
  ...
  }, [audioRef, audioDuration, waveformData, drawStaticBars, getDuration]);
