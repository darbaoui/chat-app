...
const useWaveformCanvas = (canvasRef, waveformContainerRef) => {
  ...
  const drawCursor = useCallback((context, cursorX, canvasHeight) => {
    const { height: cursorHeight, waveColor: cursorColor } =
      AUDIO_WAVEFORM_OPRIONS;
    ...
    const adjustedCursorX = Math.max(
      cursorWidth / 2,
      Math.min(cursorX, context.canvas.clientWidth - cursorWidth / 2)
    );

    context.beginPath();
    context.moveTo(adjustedCursorX - cursorWidth / 2 + cursorRadius, cursorY);
    context.arcTo(
      adjustedCursorX + cursorWidth / 2,
      cursorY,
      adjustedCursorX + cursorWidth / 2,
  ...
  const drawStaticBars = useCallback(
    (data, progress = 0) => {
      ...
      const { barWidth, barGap, progressColor, waveColor } =
        AUDIO_WAVEFORM_OPRIONS;
      ...
      data.forEach((value, i) => {
        ...
        const normalizedHeight =
          (value / maxValue) * canvasHeight * WAVEFORM_HEIGHT_SCALE_FACTOR;
        ...
        if (barWidth === 1) {
          ...
          const radius = Math.min(
            AUDIO_WAVEFORM_OPRIONS.barRadius,
            barWidth / 2,
            barHeight / 2
          );
