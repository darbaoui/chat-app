...
const useMessageStore = create((set, get) => ({
  ...
  submitMessage: async (content) => {
    ...
    const authUser = userStore.getState().user;

    let draft = draftStore.currentDraft;

    if (!draft) {
      draft = draftStore.createDraft(authUser, content);
    }

    const message = useUploadStore
      .getState()
      .uploadingMessages.get(draft.id || draft.temp_id);
  ...
  //   useUploadStore
  //     .getState()
  //     .addUploadingMessageMediaItem(draft?.id || draft?.temp_id, fileObj);
  ...
  //     useUploadStore.getState().uploadFile(fileObj, messageId);
  //   }
  // },
