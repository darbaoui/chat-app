...
const RecordAudio = forwardRef(({ updateRecordingState }, ref) => {
    ...
    const drawLiveWaveform = useCallback(() => {
        ...
        const barWidth = AUDIO_WAVEFORM_OPRIONS.barWidth;
        const gap = AUDIO_WAVEFORM_OPRIONS.barGap;
    ...
    const visualizeDuringRecording = useCallback(() => {
        ...
        // fullWaveform.current.push(Math.min(barHeight, MAX_BAR_HEIGHT));
    ...
    const pauseRecording = useCallback(() => {
        ...
        console.log('audioBlob ---->', audioBlob, audioChunksRef.current);
        setFinalAudioBlob(audioBlob);
    ...
    const handleWaveformClick = (e) => {
        if (recordingState !== 'stopped' || recordingState !== 'paused' || !audioPlayerRef.current?.duration) return;
        const canvas = canvasRef.current;
    ...
    console.log('recordingState ---1', recordingState)
